...
function loaded() {
  ...
  help_div.innerHTML="\
    ...
    <span id=\"intermediate_help\" style=\"display: none; background-color:yellow;\">move view <a href=\"javascript: keyaction('q');\">before (<font color=red>q</font>)</a> / <a href=\"javascript: keyaction('a');\"><font color=red>a</font>fter</a><br>\
    <a href=\"javascript: keyaction('c');\"><font color=red>c</font>apture</a> / e<a href=\"javascript: keyaction('x');\"><font color=red>x</font>punge</a> view<br>\
    <a href=\"javascript: keyaction('r');\"><font color=red>r</font>ename view</a>\
  </span>"
  n=views.length; k=0;
...
function set_view() {
  ...
  if (scaleY < scaleX) {		// View is vertically bound.
    ...
    mx = winW/(viewX1-viewX0); nx = -viewX0*winW/(viewX1-viewX0);
    	// y->yx*y+ny preserves proportions and maps (viewY0+viewY1)/2->winH/2:
...
function set_menu() {
  ...
  for (i=0; i<n; ++i) {
    document.viewform.viewselect.options[i] =
      new Option((i+1)+"/"+n+". "+views[i][4]);
...
function previous() {if (k>0) {--k} else {k=n-1}; goto_view(k);}

function next() {if (k<n-1) {++k} else {k=0}; goto_view(k);}
...
function drag(event) {
  ...
  if (mode == "capture") {
    rectangle_div.style.width = mouse_now[0] - mouse_start[0];
    rectangle_div.style.height = mouse_now[1] - mouse_start[1];
  } else {
...
function drag_end(event) {
  if (mode == "capture") {
    viewX0 = (mouse_start[0]-nx)/mx; viewX1 = (mouse_now[0]-nx)/mx;
    viewY0 = (mouse_start[1]-ny)/my; viewY1 = (mouse_now[1]-ny)/my;
